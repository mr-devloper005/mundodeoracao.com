...
const inputClass = 'h-12 rounded-lg border border-black/[0.08] bg-white/85 px-4 text-base font-bold text-gray-900 outline-none transition placeholder:text-gray-400 focus:border-gray-900 focus:bg-[var(--slot4-surface-bg)]'
const buttonClass = 'inline-flex h-12 items-center justify-center rounded-lg bg-[var(--slot4-surface-bg)] px-6 text-sm font-extrabold uppercase tracking-[0.22em] text-gray-900 transition hover:-translate-y-0.5 hover:bg-gray-100 disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-lg px-4 py-3 text-sm font-bold ${status === 'success' ? 'bg-emerald-50 text-emerald-800' : 'bg-red-50 text-red-700'}`}>{message}</p> : null}
