...
function SearchResultCard({ post, index }: { post: SitePost; index: number }) {
  ...
  return (
    <Link href={href} className={`group block overflow-hidden rounded-lg border border-black/[0.08] bg-[var(--slot4-surface-bg)] shadow-sm transition hover:-translate-y-1 hover:shadow-[0_18px_70px_rgba(0,0,0,0.14)] ${strong ? 'md:col-span-2' : ''}`}>
      {image ? (
        <div className={`relative overflow-hidden bg-black ${strong ? 'aspect-[16/7]' : 'aspect-[16/10]'}`}>
          ...
          <span className="absolute left-4 top-4 rounded-md bg-[var(--slot4-surface-bg)] px-3 py-1 text-[11px] font-extrabold uppercase tracking-[0.18em] text-black">{taskLabel}</span>
        </div>
      ) : null}
      <div className="p-5 sm:p-6">
        {!image ? <span className="rounded-md bg-[var(--slot4-dark-bg)] px-3 py-1 text-[11px] font-extrabold uppercase tracking-[0.18em] text-white">{taskLabel}</span> : null}
        <h2 className="mt-4 line-clamp-3 text-2xl font-extrabold leading-[0.95] tracking-[-0.06em] text-[var(--slot4-page-text)]">{post.title}</h2>
    ...
    <EditableSiteShell>
      <main className="min-h-screen bg-[var(--slot4-page-bg)] text-[var(--slot4-page-text)]">
        <section className="mx-auto max-w-[1180px] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="grid gap-8 rounded-lg border border-black/[0.08] bg-white/70 p-6 shadow-[0_12px_40px_rgba(0,0,0,0.08)] backdrop-blur md:grid-cols-[0.8fr_1.2fr] lg:p-10">
            <div>
            ...
            <form action="/search" className="self-end rounded-lg border border-black/[0.08] bg-[var(--slot4-page-bg)] p-4 sm:p-5">
              <input type="hidden" name="master" value="1" />
              <label className="flex items-center gap-3 rounded-lg border border-black/[0.08] bg-[var(--slot4-surface-bg)] px-4 py-3">
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-2">
                <label className="flex items-center gap-2 rounded-lg border border-black/[0.08] bg-[var(--slot4-surface-bg)] px-4 py-3">
                ...
                <select name="task" defaultValue={task} className="rounded-lg border border-black/[0.08] bg-[var(--slot4-surface-bg)] px-4 py-3 text-sm font-extrabold outline-none">
                  <option value="">All content types</option>
                  {enabledTasks.map((item) => <option key={item.key} value={item.key}>{item.label}</option>)}
                </select>
              </div>
              <button className="mt-3 inline-flex h-12 w-full items-center justify-center rounded-lg bg-[var(--slot4-dark-bg)] px-6 text-sm font-extrabold uppercase tracking-[0.18em] text-[var(--slot4-dark-text)] transition hover:-translate-y-0.5" type="submit">Search</button>
            </form>
          </div>
          ...
          <div className="mt-10 flex flex-wrap items-end justify-between gap-4">
            ...
            <Link href="/article" className="inline-flex items-center gap-2 rounded-md border border-black/[0.08] bg-[var(--slot4-surface-bg)] px-5 py-3 text-sm font-extrabold">Browse latest <ArrowRight className="h-4 w-4" /></Link>
          </div>
          ...
          {results.length ? (
            ...
            <div className="mt-8 rounded-lg border border-dashed border-black/[0.08] bg-white/70 p-10 text-center">
