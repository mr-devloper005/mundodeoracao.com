  ...
  return (
    <EditableSiteShell>
      <main className={`${dc.shell.section} ${dc.shell.sectionY}`}>
        <section className={`${dc.surface.card} p-6 sm:p-8`}>
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className={`inline-flex items-center gap-2 ${dc.type.eyebrow} ${pal.mutedText}`}>
                <MessageSquare className="h-4 w-4" /> Local comments
              </p>
              <h1 className={dc.type.heroTitle + ' mt-4'}>Comments</h1>
              <p className={`mt-4 max-w-2xl text-sm leading-7 ${pal.mutedText}`}>
                Review comments saved in this browser from article pages.
              </p>
            </div>
            <button type="button" className={dc.button.secondary} onClick={refreshComments}>Refresh comments</button>
          </div>

          <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="relative w-full sm:max-w-md">
              <Search className={`pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 ${pal.mutedText}`} />
              <input
                value={query}
                onChange={(event) => {
                ...
                className={`h-11 w-full rounded-lg border ${pal.border} ${pal.surfaceBg} pl-9 pr-3 text-sm outline-none`}
              />
            </div>
            <p className={`text-sm ${pal.mutedText}`}>
        ...
        {visibleComments.length ? (
          <section className="mt-8 grid gap-4">
            {visibleComments.map((item) => (
              <article key={`${item.articleSlug}-${item.id}`} className={`${dc.surface.card} p-5`}>
                <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    <p className="font-semibold">{item.name}</p>
                    <p className={`mt-1 text-xs ${pal.mutedText}`}>{formatDate(item.createdAt)}</p>
                  </div>
                  {item.articleSlug ? (
                    <Link href={`/article/${item.articleSlug}`} className={`text-sm ${pal.accentText} underline-offset-4 hover:underline`}>
                      Open article
                    </Link>
                  ) : null}
                </div>
                {item.articleTitle ? <p className="mt-4 text-sm font-medium">{item.articleTitle}</p> : null}
                <p className={`mt-3 text-sm leading-7 ${pal.mutedText}`}>{item.comment}</p>
              </article>
            ))}
          </section>
        ) : (
          <section className={`mt-8 rounded-lg border border-dashed ${pal.border} ${pal.surfaceBg} p-8 text-center`}>
            <h2 className="text-xl font-semibold">No comments yet</h2>
            <p className={`mt-2 text-sm ${pal.mutedText}`}>Add a comment on any article page and it will appear here.</p>
          </section>
        )}

        {filtered.length > COMMENTS_PER_PAGE ? (
          <div className={`mt-8 flex flex-wrap items-center justify-between gap-3 ${dc.surface.card} p-4 text-sm ${pal.mutedText}`}>
