    ...
    <section className={`${pal.creamBg} relative overflow-hidden border-b border-black/[0.06]`}>
      <div className="relative mx-auto grid max-w-[1180px] gap-10 px-4 py-12 sm:px-6 lg:grid-cols-[1.02fr_0.98fr] lg:items-center lg:px-8 lg:py-16">
        ...
        <div className="relative">
          <div className="rounded-lg border border-[var(--editable-border)] bg-white p-4 shadow-[0_18px_50px_rgba(15,23,42,0.10)]">
            <div className="rounded-md bg-[var(--slot4-accent-fill)] p-6 text-white">
              <p className="text-xs font-black uppercase tracking-[0.2em] text-white/80">Featured on {globalContent.site.name}</p>
              <h2 className="mt-5 text-3xl font-black leading-tight tracking-tight sm:text-4xl">{pagesContent.home.hero.featureCardTitle}</h2>
              <p className="mt-4 max-w-md text-sm leading-7 text-white/82">{pagesContent.home.hero.featureCardDescription}</p>
              <form action="/search" className="mt-6 flex rounded-md bg-white p-1.5 text-[var(--slot4-page-text)]">
                ...
                <button className="inline-flex items-center gap-2 rounded-md bg-[var(--slot4-dark-bg)] px-4 py-2.5 text-sm font-black text-white"><Search className="h-4 w-4" /> Search</button>
              </form>
            </div>
            <div className="grid gap-3 pt-4 sm:grid-cols-2">
              <div className="rounded-md border border-[var(--editable-border)] bg-[var(--slot4-accent-soft)] p-4">
                <p className={`text-xs font-black uppercase tracking-[0.16em] ${pal.accentText}`}>For readers</p>
                <p className="mt-2 text-sm font-bold leading-6">Cleaner article discovery, useful summaries, and steady page rhythm.</p>
              </div>
              <div className="rounded-md border border-[var(--editable-border)] bg-[var(--slot4-gray)] p-4">
                <p className={`text-xs font-black uppercase tracking-[0.16em] ${pal.accentText}`}>For writers</p>
