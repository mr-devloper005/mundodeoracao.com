  ...
  return (
    <main className={dc.shell.page}>
      <section className={`${dc.shell.section} pt-12 sm:pt-16 lg:pt-20`}>
        <div className={`rounded-lg border ${pal.border} ${pal.darkBg} p-6 text-white shadow-[0_12px_40px_rgba(0,0,0,0.08)] sm:p-8 lg:p-10`}>
          ...
          <form action={basePath} className="mt-8 flex max-w-xl flex-col gap-3 sm:flex-row">
            <select name="category" defaultValue={category || 'all'} className={`min-w-0 flex-1 rounded-md border ${pal.darkBorder} bg-[var(--slot4-surface-bg)] px-5 py-3 text-sm font-bold ${pal.panelText} outline-none`}>
              <option value="all">All categories</option>
              {CATEGORY_OPTIONS.map((item) => <option key={item.slug} value={item.slug}>{item.name}</option>)}
            </select>
            <button className={`rounded-md ${pal.accentSoftBg} px-6 py-3 text-sm font-extrabold ${pal.panelText}`}>Filter</button>
          </form>
        </div>
      </section>
      ...
      <section className={`${dc.shell.section} ${dc.shell.sectionY}`}>
        ...
        <div className="mt-10 flex flex-wrap items-center justify-center gap-3">
          {pagination.hasPrevPage ? <Link href={pageHref(page - 1)} className={`rounded-md border ${pal.border} bg-[var(--slot4-surface-bg)] px-5 py-3 text-sm font-extrabold`}>Previous</Link> : null}
          <span className={`rounded-md ${pal.darkBg} px-5 py-3 text-sm font-extrabold text-white`}>Page {page} of {pagination.totalPages || 1}</span>
          {pagination.hasNextPage ? <Link href={pageHref(page + 1)} className={`rounded-md border ${pal.border} bg-[var(--slot4-surface-bg)] px-5 py-3 text-sm font-extrabold`}>Next</Link> : null}
        </div>
      </section>
    </main>
  )
  ...
  return (
    <main className={dc.shell.page}>
      <section className={`${dc.shell.section} pt-10 sm:pt-14 lg:pt-16`}>
        <div className={`grid gap-6 rounded-lg border ${pal.border} bg-[var(--slot4-surface-bg)] p-6 shadow-[0_12px_40px_rgba(0,0,0,0.08)] lg:grid-cols-[minmax(0,1fr)_320px] lg:p-10`}>
          <div className="min-w-0">
            <Link href="/article" className={`inline-flex items-center gap-2 rounded-md border ${pal.border} px-4 py-2 text-sm font-extrabold ${pal.panelText}`}><ChevronLeft className="h-4 w-4" /> Articles</Link>
          ...
          <aside className={`min-w-0 rounded-lg ${pal.darkBg} p-6 text-white`}>
            ...
            <Link href="/contact" className={`mt-6 inline-flex items-center gap-2 rounded-md bg-[var(--slot4-surface-bg)] px-5 py-3 text-sm font-extrabold ${pal.panelText}`}>Contact <ArrowRight className="h-4 w-4" /></Link>
          </aside>
        </div>
      </section>
      <section className="mx-auto w-full max-w-5xl px-4 pb-16 pt-6 sm:px-6 lg:px-8 lg:pb-24">
        <div className={`rounded-lg border ${pal.border} bg-[var(--slot4-surface-bg)] p-6 shadow-[0_12px_40px_rgba(0,0,0,0.08)] sm:p-8 lg:p-10`}>
